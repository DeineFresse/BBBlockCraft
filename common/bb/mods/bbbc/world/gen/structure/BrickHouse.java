...
public class BrickHouse {
	...
	public static void BuildBrickHouse(World par1, int par2, int par3,
			int par4, EntityPlayer par5, int f) {

		if (!Reference.DEBUGMODE) {
			switch (f) {
			case 0: {
				south(par1, par2, par3, par4, par5);
				break;
			}
			case 1: {
				west(par1, par2, par3, par4, par5);
				break;
			}
			case 2: {
				north(par1, par2, par3, par4, par5);
				break;
			}
			case 3: {
				east(par1, par2, par3, par4, par5);
				break;
			}
			}
	...
	public static void north(World par1, int par2, int par3, int par4,
			EntityPlayer par5) {
		...
		for (int i = 0; i < 4; i++) {
			Forms.rectangle(par1, par2 - 4 + i, par3 + 3 + i, par4 - 8 + i,
					9 - 2 * i, 9 - 2 * i, BrickStair);
		}
		...
		par1.setBlock(par2, par3, par4, LoadedIDs.Block_TestBlock);

		par1.setBlock(par2, par3 + 7, par4 - 4, BrickSlab, SlabMeta, 0x01);
	}

	public static void south(World par1, int par2, int par3, int par4,
			EntityPlayer par5) {
		...
		for (int i = 0; i < 4; i++) {
			Forms.rectangle(par1, par2 - 4 + i, par3 + 3 + i, par4 + i,
					9 - 2 * i, 9 - 2 * i, BrickStair);
		}
		...
		par1.setBlock(par2, par3, par4, LoadedIDs.Block_TestBlock, 0, 0x01);

	}

	public static void west(World par1, int par2, int par3, int par4,
			EntityPlayer par5) {
		...
		for (int i = 0; i < 4; i++) {
			Forms.rectangle(par1, par2 - 8 + i, par3 + 3 + i, par4 - 4 + i,
					9 - 2 * i, 9 - 2 * i, BrickStair);
		}
	...
	public static void east(World par1, int par2, int par3, int par4,
			EntityPlayer par5) {
		...
		for (int i = 0; i < 4; i++) {
			Forms.rectangle(par1, par2 + i, par3 + 3 + i, par4 - 4 + i,
					9 - 2 * i, 9 - 2 * i, BrickStair);
		}
